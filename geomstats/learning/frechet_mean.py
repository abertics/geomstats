...
class SturmsMean(BaseEstimator):
    ...
    def procrustes_obj(self, X, mean_est, weights=None):
        """Compute sum of distances between points and mean estimate.

        Mean should minimise this objective.
        ...
        X : array-like, shape=[n_samples, *metric.shape]
            Training input samples.
        mean_est : point
            Mean estimate to compare to samples.
        weights : array-like, shape=[n_samples,]
            Weights associated to the samples.
            Optional, default: None, in which case it is equally weighted.
        ...
        if weights is not None:
            raise ValueError("Haven't implemented weights yet...")

        sum_distances = 0
        for x in X:
            aligned = self.space.aligner.align(x, base_point=mean_est)
            new_dist = self.space.metric.dist(aligned, mean_est)
            sum_distances += new_dist
        return sum_distances

    def align_and_fit(self, X, y=None, weights=None, verbose=False):
        """Compute the aligned weighted mean for geodesic metric spaces.

        Uses iterative Procrustes analysis:
            - Align all objects to current mean estimate
            - Fit new mean estimate
            - Rinse, repeat
        ...
        best_min = np.inf
        for j in range(self.align_iter):
            if verbose:
                print("NEW mean estimate", mean_estimate)
            aligned = [self.space.aligner.align(x, base_point=mean_estimate) for x in X]
            aligned = gs.array(aligned)
            if verbose:
                print("I have aligned", aligned)
            new_estimate = self.fit(aligned).estimate_
            new_prcrustes_obj = self.procrustes_obj(X, new_estimate)
            if verbose:
                print("ROUND DONE", new_prcrustes_obj, new_estimate)
            if new_prcrustes_obj < best_min:
                best_min = new_prcrustes_obj
